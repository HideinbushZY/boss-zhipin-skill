...
3. ~~参数化浏览器 ID + SAFETY.md + CONTRIBUTING.md + verify-setup + demo(#1 #15 #16 #17)~~ ✅ **已完成(2026-07-09)** —— 浏览器 ID 全占位符化、SAFETY.md/CONTRIBUTING.md 已建、`verify-setup.sh` 一键自检已加并接进 SKILL/README、`DEMO.md` 合成示例走查已加(真账号截图会泄第三方 PII,故用文字合成演示)。开源采纳最小件套已就位。
...
**#1 `verify-setup` 一键自检** · `S / 高` · ✅ **已做(2026-07-09)**
参数化浏览器 ID ✅ + `verify-setup.sh` ✅ 已加并接进 SKILL.md/README「用前必做」:一条命令查 browser-act / 浏览器 id / 登录态 / 招聘接口,输出 pass·fail,并**区分"Chrome 没接上"与"没登录"**两种失败。跨机交接第一个卡点已补。
...
**#17 README 补 FAQ + 演示 + 国际化标注** · ✅ **已做(FAQ 2026-07-06 / DEMO 2026-07-09)** —— README 加了 10 条 FAQ;`DEMO.md` 合成示例走查已加(两种触发 + 安全刹车演示;**刻意用文字合成而非真账号截图/GIF——真账号页面全是第三方求职者 PII,截图=泄露**)。剩:GitHub About/topics 由仓主在网页端设(见 README 建议文案)。

**#19 候选人意图硬门 + 三层自治错题本 + report_first 安全默认** · ✅ **已做(2026-07-14)** —— Phase 0 候选人意图硬门(`scripts/notebook.py gate-action`,五类外发前必过;`reject/no_interest/do_not_contact` 硬阻断;新回复 `pending_intent_review` 未确认 `interested` 前 fail-closed;独立于错题本且解不开)+ 本地 per-user PII-free 错题本三层自治(auto/confirm/note_only,只收权不扩权、PII 拦截、损坏 fail-closed)+ 触达默认档改 `report_first`(零外发,升档需授权)。prose(SKILL/playbook/operation-map/SAFETY)、代码(`notebook.py`/`validate.py`)、schema、tests 全对齐。惰性过期遇坏 `ttl_days` 也 fail-closed(坏行归 corrupt、不崩溃)。
