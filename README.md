...
> A reusable Claude skill that turns an agent into a Boss直聘 (Chinese recruiting platform) **recruiter-side** operator: it drives a real logged-in Chrome to search/greet/screen candidates, and runs a strategy-driven sourcing pipeline (parse JD → find → LLM-score → reach out → report) with a local ledger for incremental runs. Most page maps and selectors were verified by live operation on a real account (the few pending/unverified items are flagged in `operation-map.md` §8). Requires browser-act + a logged-in Boss直聘 **enterprise recruiter** account (see Prerequisites).

一个可复用的 Claude skill:让 agent 像 **Boss直聘招聘者侧的熟手** 一样操作工作台,并把一段"搜索策略"跑成 `解析→找人→打分→触达→报告` 的自动寻访管线。页面地图和选择器**绝大多数**真机实测过(少数待补/未跑通的在 `operation-map.md` §8 标注)。**需自备 browser-act + 一个已登录的 Boss直聘企业招聘者账号**(见「用前提」)。
...
**Q: 搜索畅聊卡"开聊"怎么这么贵?**
A: **逐人消耗 1~3 张**(不是固定 3、也不是 1;开聊前读接口 `searchChatCardCostCount` 得知该人成本),而且自动"发起沟通 + 索要简历/微信/电话"(捆绑 PII 请求,碰红线)。余量走接口 `geeks.json` 的 `chatCardCount`(别再靠读 DOM)。用前务必知情——见 `SAFETY.md`。
