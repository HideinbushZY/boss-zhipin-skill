...
**永久红线(任何档、任何模式都不自动,只报告建议)**:换电话 / 换微信 / 约面 / **发布·删除职位** / 举报 / 批量标记不合适(均不可逆/对外/深PII)。`budget.chat_cards=0` 时不碰卡。
**关闭职位=可逆,可代操作**(非红线):可重新开放、数据保留;硬门=先核对岗位+关前确认+关后验证(见 operation-map §4.1/§6)。删除仍红线。
