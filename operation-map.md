...
- **✅ 正确的定点搜索法**:① **清污染**——接口 `jobId=0`(纯关键词)/ UI 职位选择器设「不限职位」;② 用**该人独特的关键词**(公司/产品/项目名,如「小度助手」「荣耀 语音助手」「8295」;**别用"语音交互"这种大词**)→ 目标精准置顶。实测 `geeks.json?jobId=0&keywords=小度助手&source=1` → 某(某小度产品负责人)**排第1**;`荣耀 语音助手` → 某排第6。**关键词确实过滤**(旧记"不过滤"为误,那是被职位污染+大词淹了)。
