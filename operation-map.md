...
> **本文 URL / 选择器 / 流程绝大多数为实测,非推演**(极少数未跑通项见 §8「仍待补」清单)。页面改版后需重新验证。
...
- **§0** 关键技术事实必读 · **§1** 页面地图 · **§3** 候选人漏斗/状态机 · **§5** 成本额度 · **§6** 安全门/红线 · **§7** 踩坑速查表 · **§8** 进度清单
...
> **⑤ 关键词框** `.search-input`(默认空):输入触发 autoSuggest 下拉——**按原词搜就点 `i.icon-search`,别点 suggest 项**;**写入必须用 eval 原生 value setter + input 事件(§7i;browser-act `input` 写不进)**。
...
- **✅ 清默认三件套·确切选择器(2026-07-05 实测补齐,§8 待补清零)**:
  - **关键词框 = `.search-input`(2026-07-09 更正写入法,见 §7i)**:**browser-act `input <idx>` 索引不到、写不进这个框**(旧记的 `input <idx>` 是误,当初正是它让"词没进框、搜了残留词"→ 误判"关键词不过滤")。**正确写法 = eval 原生 value setter + 派发 input 事件**:`var st=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set; st.call(k,'你的词'); k.dispatchEvent(new Event('input',{bubbles:true}))`(注意:普通 `el.value=` 会被 React/Vue 重渲染冲掉,但**原生 setter+input 事件不会**——这是框架兼容赋值)。写完点 `i.icon-search` 触发(⚠ 它的 state 索引关弹层后会漂,每次重 grep)。清空同法 set 空串。
...
- 关键词框 `.search-input` + `i.icon-search` 执行(输入会弹 suggest 下拉,按原词搜就点 icon,别选下拉)。**关键词写入用 eval 原生 value setter + input 事件(§7i)**;城市框是另一个 `input`(在 `.search-city-kw` 内)。
...
- 职位卡 **`.job-item-container`**,标题 **`.job-title`**(2026-07-08 MCP server 复测更正;旧记 `li.job-jobInfo-warp` 已失效)。卡在**职位列表 iframe 内**(同源 eval 要遍历 iframe.contentDocument 找 `.job-item-container`)。含:标题 + 城市/经验/学历/薪资/类型 + 数据块(看过我/沟通过/感兴趣)+ 状态 + 到期日。
...
- **✅ 关闭职位流程(可操作,2026-07-07 补)**:关闭是**可逆**动作——关掉的岗进「已关闭」页签,点「重新开放」即可复活,**候选人对话/简历/漏斗数据全保留**;故不列永久红线(删除才是不可逆红线)。操作:
  ...
  - `.job-item-container` 在 iframe 里,读卡用同源 eval;**确认框弹在主 document(非 iframe)**。
  - **✅ 2026-07-08 真机 close+reopen 实测(净零还原)**:确认框确认键=**`span.boss-dialog__button`,文本恰为「关闭职位」**(排除「暂不关闭」;`find_index` 索引不到,用 eval 按文本找+点);**重开按钮文本=「打开」**(不是「重新开放」);**🔴 重开会立刻弹「持续曝光7天/14天版…支持开发票」曝光推销弹窗——必须 × 拒(绝不购买,「只关不买曝光」红线)**。
...
**🔴 securityId 每次搜索临时分配、跨搜索/跨时间会变(2026-07-09 固化,重要根因)**:同一个人在两次不同搜索里拿到的 securityId **不一样**。两个后果:
- **批量采集/去重别拿 securityId 当键**——同一人被多个关键词各搜到一次、每次 sid 不同,按 sid 去重**会漏**,把一个人当成多人灌一堆重复(实测把 18 个唯一的人灌成 82 条)。**去重用稳定签名 = 掩码名 + 年龄 + 工作履历(公司/岗位/起止)**。
- 这也是"存下某人 sid、隔天想精确复现或再触达他"不可靠的**根因**(见 §7i-复盘):sid 已变,旧 sid 复用不了 → 重新触达要用**该人独特标记的多词签名 re-surface**,不是复用 sid。
...
## 7h. ✅ 以人找人(样本简历 → 相似候选人,只读;2026-07-08 实测跑通)

**触发**:用户给 **1+ 份**"觉得匹配"的候选人简历(PDF/文本),要在 Boss 上找一批类似的人。纯只读(搜索 + geek/info 详情),**零外发、零红线、零成本**。

**样本数量不定,别假设**:可能 1 份、也可能 N 份"合适简历",不确保用户手头有几份。**N 份 → 取画像交集;1 份 → 直接照该人抽画像**。

**流程(5 步,全是既有能力的组合,不是新焊的固定管线):**

1. **提画像(LLM 语义抽,不写死)**:读样本简历 → 抽三层:
   - **硬指标**:年限区间 / 学历 / 城市 / 薪资带 —— 用于后置打分,**不一定进接口硬筛**。
   - **关键词集**:能把这些人"钉住"的域词(如 语音交互 / 语音助手 / AI语音 / 智能硬件 / Agent记忆);**多份取并集**。
   - **软画像**:大厂语音助手/车机背景、软硬结合、多端、C端体验等语义特征 —— 交给 LLM 打分用,**不量化权重**。

2. **多词扫描 + 合并去重**:关键词集里**每个词各搜一趟** `geeks.json`(§7e),按 `securityId` 去重并池。
   - **jobId 用污染规律(§2B)**:关键词与某在招岗语义**近** → 挂该岗 `encJobId` 聚焦;纯泛词/无对应岗 → `jobId=0`。
   - **接口筛选放松**(通常只锁城市),年限/学历/薪资**留到打分阶段** —— 否则硬筛会漏掉"1年但大厂高潜"这种。
   - **⚠ 搜索排序漂移(实测,重要)**:**同参数不同趟,结果集和顺序都会变**。→ **要用某人(读详情/导简历)就在拿到他的同一趟里存下 `securityId` 立刻用掉;别隔次靠"学校+年限"这类属性回找,会找不回**(实测隔趟丢了 3/9 目标)。

3. **列表粗筛**:池里用 `geekCard` 的 `geekDesc`/年限/学历/城市/期望初排,砍明显离题的。

4. **详情深读定档(必做,不能省)**:短名单里的人逐个 `geek/info`(§7e,破 canvas)拿**真实公司/职级/时长/全文**。
   - **⚠ `geekDesc` 会美化误导(实测)**:有人列表自述像"全栈AI产品",深读是电商转岗;有人列表只写一句,深读炸出大厂车机+硬件履历。**排名必须基于 geek/info 的真实履历,不能信列表自述。** geek/info 免费只读,短名单**全深读没成本**。

5. **LLM 对画像打分排档(不固化 rubric)**:把每人**真实履历**丢给 LLM,对着软画像做**语义贴合判断**,排 T1/T2/T3 + 标贴合点 + 风险标记(薪资异常/资历过浅等)。**不写死权重、不量化打分公式** —— 保持 LLM 判断。

**输出**:排档短名单 + 每人一份简历 markdown(§7g,来源解耦)。**只读不触达**;真要联系走正常触达门(打招呼/畅聊,红线不变)。**要定点触达这份名单里的某几个人是可行的**:用每人独特关键词(公司/产品名)清污染(jobId=0)精准搜到 → UI 开聊(§7i 定点搜索法;已开聊者会自动从搜索隐藏)。
...
## 7i. ✅ 搜索开聊(畅聊)执行机制 + 定点搜索法(2026-07-08 实测 / 2026-07-09 更正定点结论)
...
**成本 / 余量(修正旧记)**:
- **单次开聊成本 = 逐人不同 1~3 张**,读 geekCard 的 **`searchChatCardCostCount`**(非固定 3)。
- **畅聊卡余量接口(§7d 一直缺的,现定位)**:`geeks.json` 响应顶层 **`chatCardCount`**(= 剩余)/`allChatCardCount`(总)。做卡预算直接读它,别再靠 DOM「剩余次数 xN」。

**🟢 定点搜索一个特定候选人 = 可行(2026-07-09 更正,推翻旧"无法定点"结论)**
> ⚠ 旧版这里写"搜索无法定点触达指定的人"**是错的**——用户实测:**清污染 + 该人独特关键词**就精准搜到并开聊了目标。我旧结论错在**四点叠加**:①带着职位 scope 搜(结果被岗位排序主导、关键词被淹)②只用宽泛词(如"语音交互")③**不知道"已开聊者会从搜索页隐藏"**(见下),把"搜不到"误当"随机池"④UI 关键词根本没进框。

- **✅ 正确的定点搜索法**:① **清污染**——接口 `jobId=0`(纯关键词)/ UI 职位选择器设「不限职位」;② 用**该人独特的关键词**(公司/产品/项目名,如「小度助手」「荣耀 语音助手」「8295」;**别用"语音交互"这种大词**)→ 目标精准置顶。实测 `geeks.json?jobId=0&keywords=小度助手&source=1` → 目标(某小度产品负责人)排第1。**关键词确实过滤**(旧记"不过滤"为误,那是被职位污染+大词淹了)。
  - **🔑 UI 关键词输入法(2026-07-09 实测,之前误判的直接根因)**:搜索关键词框是 iframe 里的 **`.search-input`**,**browser-act `input <idx>` 索引不到、写不进**(我旧测试就是这样没进框、搜的残留词,才误得"关键词不过滤")。**正确写法 = eval 用原生 value setter + 派发 input 事件**:`var st=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set; st.call(k,'关键词'); k.dispatchEvent(new Event('input',{bubbles:true}))`(标准 input 的 Vue 兼容赋值;≠ 消息框那种 contenteditable 要 execCommand)。写完点 `icon-search` 触发。
  - **⚠ 搜索键索引会漂**:关弹层/开聊后 `icon-search` 的 state 索引会变,每次搜索前**重新 grep 一遍**再点(旧索引会点空、搜索不重跑、看到的是上次结果)。
  - **选词启发式(2026-07-09 摸清)**:定点法**对"独特且突出"的关键词极好用**;排序看关键词在**个人优势摘要 `geekDesc`** 里的突出度——**用其当前公司/旗舰产品**(在摘要里),别用**埋在工作史深处的旧东家**(如"小天才""岚图"是过往经历、摘要没突出 → 排不到前面);**思必驰这种一堆人共有的公司**也难单独隔离。**2 个关键词收窄、太多反而稀释**(「华为耳机 荣耀 语音」→目标#1;「岚图 比亚迪 思必驰」三词反而糊)。
- **🔑 已开聊候选人从搜索隐藏(Boss 内建去重,重要)**:用畅聊卡开过聊的人**不再出现在搜索结果里**。→(a)平台自带去重、省心,不用自己滤已聊的;(b)但"搜不到某人"可能是"已经聊过了"而非"没这人"——**别据此误判**。
- **触达仍走 UI**:`useCard` raw XHR 有反 CSRF(直调返 HTML、不扣卡),开聊那一下必须走页面 UI 点击(定点搜到 → 卡 hover btn-chat → 选岗 → 确认)。**但"找人"已不是问题**——独特关键词(jobId=0)一搜即得,不再靠随机抽池。
- **修正结论**:**定点触达一个/一批指定候选人 = 可行**:独特关键词 jobId=0 精准定位 → UI 逐个开聊。旧"只能机会主义、凑不齐名单"作废(根因是我用大词+带污染+没意识到已聊隐藏,不是平台做不到)。
- **🔴 定点开聊仍要过成本/PII 确认闸(别因为"搜到就点开聊"漏掉)**:开卡=耗畅聊卡(读 `searchChatCardCostCount`)+ 自动索要简历/微信/电话(PII 捆绑)——属**消耗权益 + 外发红线**(§6/§2B)。**开卡前必须先报卡数、告知 PII 捆绑、等用户点头**;批量定点=**逐人各过一次此闸**,不得凑齐名单后连点。定点法降低的是"找到人"的摩擦,**没降低触达授权门槛**。

### 7i-复盘. 搜索开聊·执行层实战复盘(2026-07-09 批量定向触达一波资深人选后总结)

**🎯 触达"一个指定的人" = 多词签名法(最可靠的定点触达,2026-07-09 用户实测固化)**
要触达某个你**已锁定的特定候选人**(不是"在池子里随便找个合适的"),别用单个宽词——用**该人 2-3 个最独特标记的 AND 组合**当搜索关键词:
- **选词**:当前公司 + 旗舰产品/项目 + 一个过往东家(越独特越好,如「某产品名 + 某前东家 + 某现公司」)。单个词(哪怕独特公司)常把目标淹在同名/同司的一堆人里;**2-3 个独特词叠加,把他精准收窄到结果首位**。
- **接口评估与 UI 触达用同一组词**:geeks.json 评估用哪组词,searchFrame 触达就用同一组(单词时两边常是两套人;同组多词才对齐)。
- **命中后照常**:读全简历确认 → 卡上多特征强签名核身 → 选级别相称的岗 → 验 `li.active` → 开聊。
- **🔴 反面教训**:曾**单搜一个产品名**、UI 返回 0 卡,就误判"这人 Boss 触达不到";换成**三词组合(该产品+某前东家+某公司)一搜即置顶、开聊成功**——**单词搜不到 ≠ 触达不到**。


- **🔴 XHR 评估 与 UI 触达必须用同一个关键词**:`geeks.json`(接口读)和 `searchFrame`(UI 开聊)对**同一个词返回不同的人/顺序**。若用接口 A 词读到某人评估、却用 B 词(哪怕是他独特的公司名)去 UI 搜,**大概率渲染不出他的卡、根本触达不到**。→ 接口用哪个词评估,UI 就用**同一个词**开聊。(踩过:接口用某公司名读到目标,UI 换成其子公司名搜就没渲染出来。)
- **"当场认人当场触达"最顺,但离线清单也能精确复现——靠"多词收窄"(2026-07-09 用户实测更正:原写"复现不可靠"说过头了)**:名字打码 + **单个宽词**搜索时排序每次变 → 单词确实很难把某个已评估的人刷回 UI(我曾据此误判"UI 触达不到")。**但用该人独特标记的多词 AND 组合(独特公司+产品+过往东家,2-3 个叠加)能把他精准收窄到 UI 前列、可靠触达**(实测:某人单搜一个产品名 UI 出 0/不在前列,换"该产品+某前东家+某公司"三词一搜即置顶)。→ **离线深挖 N 个 → 逐个用多词签名 re-surface → 触达,是可行的**;别像我一样单词搜一次没出来就误判。可靠动作仍是:搜 → 读全简历 → 强签名核身 → 当场开聊。
- **选岗弹层 = 招呼语框架的控制点(高价值)**:搜索开聊的系统首句 = "诚聘【你在选岗层选中的岗】一职"。**选岗决定招呼语调性**——对资深/高端人选,若账号里有**级别相称的预设岗**(如"创业合伙人 / 资深负责人"类在招岗)就选它,招呼语才不违和、回复率才不崩;随手用个低阶岗 = 群发招聘话术,高端人选直接不理。**所以选岗不是随便点默认。**
- **🔴 选岗弹层有默认预选,≠ 你要的岗;确认花卡前必验选中态**:弹层会默认预选一个岗。点你的目标岗**可能不生效**——要**点准那一行 `<li>`**(点到子元素不切换)。**花卡前必须回读 `li.active` = 你选的岗**,否则招呼语发错岗、调性全错(真踩过:默认预选停在别的岗,第一次点没切过去)。⚠ 弹层渲染有 **~3-4s 延迟**,btn-chat 后别立刻读/选,等它出来。
- **花卡前必读全简历 + 强签名核身(卡摘要会骗人)**:卡片摘要看着匹配,点进去读 `geek/info` 全简历常常不是那回事——**关键词会撞车**:搜某公司名撞到"在那当员工的人",而你要的是"在那创过业的目标人";或同姓不同人;或卡摘要按搜索词高亮的那段掩盖了真实履历方向。→ **触达前必读全简历确认匹配 + 用多特征强签名(掩码名+年龄+独特公司/经历短语 叠加)核身**,别只凭卡摘要就花卡。这是"别扣错卡 / 别把 PII 发错人"闸的**具体失败模式**。
- **两个小坑**:① btn-chat 上标的畅聊卡成本可能是 **stale**(显示"3/65"而实际余额已 61)——信"确认按钮 `搜索畅聊卡(N/M)`"上的实时数;② **资深/创始级广搜用 `city=不限`**(searchFrame URL `city=-2`)——这种人地域分散,单城市池薄、放开地域池肥很多。

## 7j. ✅ 推荐通道触达自动化(2026-07-08 系统实测)—— 免费、可靠的"扫池群发",但不是"定点名单"

**为什么推荐是触达自动化的主力通道**:免费(不耗畅聊卡)、真名(不打码)、卡自足(无 canvas)、有 haveChatted 去重标、打招呼一键无确认。

**可靠的自动化模型 = "扫池群发"(不是"发给指定名单")**:
1. **枚举**:`GET /wapi/zpjob/rec/geek/list?jobId={encJobId}&page=N&age=16,-1&degree=0&experience=0&salary=0&keyword1=-1&gender=0&major=0&intention=0&activation=0&cardType=0` → `zpData.geekList[]`(15/页,`hasMore`)。geekCard 有 `geekName`(**真名**)、securityId、geekWorkYear/Degree、salary、expectPositionName;geek 级 `haveChatted`/`isFriend` = 去重标。
   - **⚠ 关键:一趟连续翻页(page 1→N)是连贯的**(实测 8页120条、去重120、0重复);但**重排只发生在"重新拉 page1"之间**(连拉两次 page1 前5名完全不同)。→ **枚举整池必须一口气 1→N 扫完,别中途回 page1(会重排)。**
2. **去重**:`haveChatted==1||isFriend==1` 跳过(免重复触达风控,§7f)。
3. **打招呼(免费,一键)**:recommendFrame 卡上 `button.btn-greet`(直接可见、非 hover-gated)→ 点即发,**无确认弹层**,发系统模板"你好,我司急聘{岗位}一职,请问考虑么?"→ 按钮变「继续沟通」(§7c 已验证)。走标准沟通额度、**不扣畅聊卡**。
4. **无批量**:页面无"全选/批量打招呼",`batchChat/panel` 空 → 只能**逐卡点 btn-greet 循环**(打招呼接口是 Vue handler,未单独抓;无批量端点,逐个 UI 点即可)。
5. **定制开场白**:打招呼后「继续沟通」→ 进会话 → `execCommand` 填 `.bosschat-chat-input` 补发(同 §7i 第5步)。
6. **额度闸**:日限 200 沟通,读 `todayData.chatInitiative`(§7d);扫池群发前先算 `200−已用` 够不够本轮。

**能/不能**:
- ✅ **能**:"把某岗推荐池里没聊过的人,免费打招呼一遍(到额度上限)"——可靠、可循环(每天推荐会补新人)。这是真正能跑的触达自动化。
- ⚠ **推荐通道本身不适合"定点某个指定的人"**(推荐池是算法按岗选的、每次重排,你要的特定人未必在里面)。**但要定点触达指定名单,走搜索通道的定点搜索法(§7i:独特关键词 jobId=0)完全可行**——推荐负责"免费扫合格池群发",搜索负责"定点找某个特定人",两条各司其职。
- **一句话**:**推荐 = 免费扫池群发引擎**(顶漏斗走量)· **搜索定点法(§7i)= 精准找特定人**(定向外发)。策略寻访(playbook)的走量自动触达架在推荐上;要 pitch 某几个指定的人走搜索定点。
...
- [x] **定点搜索+触达法**(清污染 jobId=0 + 独特关键词精准定位 → UI 开聊;UI 关键词框 `.search-input` 用原生 setter 写;已开聊者自动隐藏;2026-07-09 更正旧"无法定点"错判 + 定点法实测触达3人,见 §7i)
